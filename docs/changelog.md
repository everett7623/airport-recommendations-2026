...
### 更新
- 🔄 同步 VPSKnow `2026-08-11` 数据，新增超级飞鱼并重新生成机场推荐文档。

### 修复
- 🐛 同步版本优先读取 VPSKnow 实际机场数据文件的 `Updated` 日期，页面头日期仅作兼容兜底；缺失上游版本时直接失败，不再使用工作流运行日期。
...
### 更新
- 🔄 同步 VPSKnow `2026-08-04` 数据，新增财路云并刷新当前机场资料。
- 🔄 新增 `accessType` 字段及 `access_type` 受控词汇，将通用订阅、专用客户端和两者兼容状态同步到 JSON 与两份 README。
- ✅ 校验器新增接入方式合法值和词汇表完整性检查。

---

## [2026-07-21] - 同步规则与数据更新

### 更新
- 🔄 同步 VPSKnow `2026-07-21` 数据：Sogo云纳入当前下架名单，寰宇云因运营方变化从重点推荐调整到完整总榜待复核。
- 🔄 将 `docs/sync-setup.md` 设为同步规则的权威文档，明确上游 ref、版本语义、四个必需同步文件、条件文档和验收清单。
- 🔄 对齐 `AGENTS.md`、`CONTRIBUTING.md` 与 Pull Request 模板中的同步检查项。
- 🔄 GitHub Actions 统一在生成派生文档后执行 `npm run validate`，与本地验收入口保持一致。

### 修复
- 🐛 生成文档校验改为忽略 CRLF/LF 差异，避免 Windows 本地校验误报 README 过期。
...
### 更新
- 🔄 项目许可从 MIT 调整为 CC BY-NC-SA 4.0，明确原创内容、数据和文档的非商业共享条件。
- 🔄 README 与精简版 README 新增统一的许可、商业使用和推广利益披露。
